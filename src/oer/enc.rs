...
impl<'a, const N: usize> Default for Encoder<'_, N> {
    fn default() -> Self {
...
impl<'a> Buffer<'_> {
    ...
    fn clone(&self) -> Vec<u8> {
        match self {
            Self::Borrowed(buf) => buf.borrow().to_vec(),
            Self::Owned(buf) => buf.borrow().clone(),
        }
    }
    fn len(&self) -> usize {
        match self {
...
#[derive(Debug)]
pub struct Encoder<'a, const N: usize = 0> {
    ...
    field_bitfield: LinearMap<(usize, Tag), (FieldPresence, bool), N>,
...
impl<'a, const N: usize> Encoder<'a, N> {
    ...
    // }
    pub fn set_bit(&mut self, tag: Tag, bit: bool) {
        // self.field_bitfield.get_mut(&(0, tag)).unwrap().1 = bit;
        if self.options.set_encoding {
            match self.field_bitfield.get_mut(&(usize::default(), tag)) {
                Some((_, b)) => *b = bit,
                None => (),
            }
        } else {
    ...
    fn new_set_encoder<const FC: usize, C: Constructed<FC>>(&self) -> Encoder<FC> {
        let mut options = self.options;
        options.set_encoding = true;
        let mut encoder = Encoder::<FC>::new(options);
    ...
    fn new_sequence_encoder<const FC: usize, C: Constructed<FC>>(&self) -> Encoder<FC> {
        let mut encoder = Encoder::<FC>::new(self.options.without_set_encoding());
        encoder.field_bitfield = C::FIELDS
            .iter()
            .enumerate()
    ...
    fn encode_constructed<const FC: usize, C: Constructed<FC>>(
        &mut self,
        tag: Tag,
        mut encoder: Encoder<FC>,
    ) -> Result<(), EncodeError> {
...
impl<'a, const N: usize> crate::Encoder<N> for Encoder<'a, N> {
    type Ok = ();
    ...
    fn encode_sequence<const FC: usize, C, F, T>(
        &mut self,
        tag: Tag,
        encoder_scope: F,
    ) -> Result<Self::Ok, Self::Error>
    where
        T: crate::enc::Encoder<FC>,
        C: Constructed<FC>,
        F: FnOnce(&mut Encoder<FC>) -> Result<(), Self::Error>,
    {
        // let mut encoder = self.new_sequence_encoder::<FC, C>();
        let mut encoder = Encoder::<FC>::new(self.options.without_set_encoding());
        encoder.field_bitfield = C::FIELDS
            .iter()
            .enumerate()
        ...
        self.encode_constructed::<FC, C>(tag, encoder)?;
        Ok(())
    }
    ...
    fn encode_sequence_of<E: Encode>(
        ...
        let mut encoder = Encoder::<0>::from_buffer(self.options, &self.output.inner());
        for one in value {
    ...
    fn encode_set<const FC: usize, C, F, T>(
        &mut self,
        tag: Tag,
        encoder_scope: F,
    ) -> Result<Self::Ok, Self::Error>
        ...
        F: FnOnce(&mut Encoder<FC>) -> Result<(), Self::Error>,
    {
        ...
        let mut encoder = Encoder::<FC>::new(self.options);
        encoder.field_bitfield = C::FIELDS
            .canonised()
            .iter()
            .enumerate()
        ...
        E: Encode + Constructed<N>,
    {
        ...
        self.set_bit(E::TAG, true);
        let mut encoder = self.new_sequence_encoder::<N, E>();
...
mod tests {
    ...
    fn test_integer_with_length_determinant() {
        ...
        assert_eq!(encoder.output.into_inner(), v);
    }
    #[test]
