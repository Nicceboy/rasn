...
impl<'a> Buffer<'_> {
    ...
    // fn inner_mut_borrow(&mut self) -> core::cell::RefMut<Vec<u8>> {
    //     match self {
    //         Self::Borrowed(buf) => buf.borrow_mut(),
    //         Self::Owned(buf) => buf.borrow_mut(),
    //     }
    // }

    // fn as_ref(&self) -> &[u8] {
    //     match self {
    //         Self::Borrowed(buf) => buf.borrow().as_ref(),
    //         Self::Owned(buf) => buf,
    //     }
    // }
}
...
pub struct Encoder<'a> {
    ...
    field_bitfield: LinearMap<(usize, Tag), (FieldPresence, bool), 10>,
