...
impl<'output> Encoder<'output> {
    ...
    #[must_use]
    pub fn new(options: EncoderOptions, output: &'output mut Vec<u8>) -> Self {
        Self {
            options,
            // output: Vec::with_capacity(PrimitiveInteger::BYTE_WIDTH * 2),
            output,
    ...
    fn extend(&mut self, tag: Tag) -> Result<(), EncodeError> {
        match self.output.len().checked_add(self.output.len()) {
            Some(_) => {
                if self.options.set_encoding {
                    self.set_output.insert(tag, self.output.clone());
                    self.output.clear();
                } else {
                    // self.output.extend_from_slice(&self.output);
                }
            }
        ...
        match octets.cmp(&needed) {
            Ordering::Greater => {
                if signed && value.is_negative() {
                    // 2's complement
                    self.output
                        .extend(core::iter::repeat(0xff).take(octets - bytes.len()));
                } else {
                    self.output
                        .extend(core::iter::repeat(0x00).take(octets - bytes.len()));
                }
            }
            Ordering::Less => {
    ...
    fn new_set_encoder<C: Constructed>(
        options: EncoderOptions,
        output: &'output mut Vec<u8>,
        length: usize,
    ) -> Encoder<'output> {
        // let mut options = self.options;
        let mut encoder = Self::new(options, output);
        encoder.field_bitfield = C::FIELDS
            .canonised()
            .iter()
            .map(|field| (field.tag_tree.smallest_tag(), (field.presence, false)))
            .collect();
        encoder.parent_output_length = Some(length);
        encoder
    }

    fn new_sequence_encoder<C: Constructed>(
        options: EncoderOptions,
        output: &'output mut Vec<u8>,
        length: usize,
    ) -> Encoder<'output> {
    ...
    fn encode_constructed<C: Constructed>(
        ...
        self.output.extend(crate::bits::to_vec(&preamble));
        ...
        let mut extension_bitmap_buffer = BitString::with_capacity(30);
        ...
        self.output
            .extend(crate::bits::to_vec(&extension_bitmap_buffer));
    ...
    // fn ext_fn_encode() -> Resut<> {
    //
    // }
}

impl<'output> crate::Encoder for Encoder<'output> {
    type Ok = ();
    ...
    fn encode_bool(&mut self, tag: Tag, value: bool) -> Result<Self::Ok, Self::Error> {
        self.set_bit(tag, true);
        self.output
            .extend(alloc::vec![if value { 0xffu8 } else { 0x00u8 }]);
        self.extend(tag)?;
        Ok(())
    }
        ...
        if let Some(size) = constraints.size() {
            ...
            if size.constraint.is_fixed() && size.extensible.is_none() {
                ...
                if size.constraint.contains(&value.len()) {
                    ...
                    self.output
                        .extend(crate::bits::to_vec(&bit_string_encoding));
                } else {
                    return Err(EncodeError::size_constraint_not_satisfied(
                        value.len(),
                        &size.constraint,
    ...
    fn encode_enumerated<E: Enumerated>(
        ...
        if 0isize <= number && number <= i8::MAX.into() {
            self.encode_constrained_integer_with_padding(1, &number.into(), false)?;
            // self.output.extend(bytes);
        } else {
    ...
    where
        C: for<'b> Constructed,
        F: FnOnce(&mut Encoder<'output>) -> Result<(), Self::Error>,
    {
        let options = self.options.without_set_encoding();
        let length = self.output_length();
        let mut encoder = Encoder::new_sequence_encoder::<C>(options, &mut self.output, length);
    ...
    fn encode_sequence_of<E: Encode>(
        ...
        self.set_bit(tag, true);
        self.encode_unconstrained_integer(&value.len().into(), false)?;
        for one in value {
        ...
        F: FnOnce(&mut Self) -> Result<(), Self::Error>,
    {
        ...
        let length = self.output_length();
        let mut set = Self::new_set_encoder::<C>(options, self.output, length);
    ...
    fn encode_choice<E: Encode + Choice>(
        ...
        let tag: Tag;
        {
            let mut choice_encoder =
                Encoder::new(self.options.without_set_encoding(), &mut self.output);
            tag = encode_fn(&mut choice_encoder)?;
            buffer = choice_encoder.output();
        }
        ...
        if is_root_extension {
            self.output.append(&mut buffer);
        } else {
            self.encode_length(buffer.len())?;
            self.output.append(&mut buffer);
        }
        self.extend(tag)?;
        Ok(())
